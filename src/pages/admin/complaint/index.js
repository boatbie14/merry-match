            ...
            {packages.map((pkg, index) => (
              <tr key={pkg.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  {pkg.icon_url ? (
                    <Image src={pkg.icon_url} alt={pkg.package_name} width={24} height={24} className="rounded-md object-cover" />
                  ) : (
                    <div className="w-6 h-6 bg-gray-200 rounded-md" />
                  )}
                </td>
                <td className="px-4 py-3 font-medium">{pkg.package_name}</td>
                <td className="px-4 py-3">{pkg.merry_per_day !== null ? `${pkg.merry_per_day} Merry` : "ไม่จำกัด"}</td>
                ...
                <td className="px-4 py-3 text-center flex gap-3 justify-center">
                  <button onClick={() => router.push(`/admin/edit-package?id=${pkg.id}`)} className="text-pink-500 hover:text-pink-700">
                    <FiEdit size={18} />
                  </button>
                  <button onClick={() => handleDelete(pkg.id)} className="text-pink-500 hover:text-pink-700">
                    <FiTrash2 size={18} />
                  </button>
                </td>
              </tr>
