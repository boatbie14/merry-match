...
export default function LoginPage() {
  ...
  const { updateLastActiveAsync } = useUpdateUserActivity();
  ...
  async function logIn(e) {
    ...
    if (!isEmail) {
      ...
      const { data: userByName, error: userNameError } = await supabase.from("users").select("email").eq("username", emailOrName).single();
    ...
    const { data: userRecord, error: userError } = await supabase.from("users").select("user_role").eq("id", userId).single();
      ...
      <main className="min-h-screen flex flex-col md:flex-row bg-[#fdfafd]">
        ...
        <div className="hidden md:flex md:w-1/2 items-center justify-center p-6">
          <div className="relative rounded-full overflow-hidden w-[380px] h-[500px] sm:w-[420px] sm:h-[540px] lg:w-[460px] lg:h-[580px] xl:w-[500px] xl:h-[620px] bg-white">
            <Image src={loginImage} alt="Login visual" fill className="object-cover object-right" />
          </div>
        </div>
