          ...
          {packagInSubscriptions?.stripe_subscription_id && (
            <div className="w-full md:px-5 mt-21">
              ...
              <CreditInfomationCard
                ...
                expire={`${infomationPackageCard?.exp_month < 10 ? "0" : ""}${infomationPackageCard?.exp_month}/${
                  infomationPackageCard?.exp_year
                }`}
                last4={infomationPackageCard?.last4}
                editPaymentMethod={() => {
                  editCreditCard(packagInSubscriptions?.stripe_subscription_id);
                }}
              />
            </div>
          )}
            ...
            <div className="w-full md:max-w-[1246px] md:px-7 md:pb-[112px]">
              ...
              <div className="md:border border-[#D6D9E4] md:rounded-4xl bg-[#FFFFFF] md:p-8 md:pb-3  max-w-full mx-auto text-sm text-[#3C3C4399] md:mx-4">
                <div className="font-semibold text-[#646D89] text-[20px] mb-2 md:mb-1 md:pt-1 px-4 md:px-0">
                  {packagInSubscriptions?.canceled_at ? "Subscription ends on" : "Next billing"} :{" "}
                  <span className="text-[#646D89]">{formatDateToUserTimezone(packagInSubscriptions?.current_period_end)}</span>
                </div>
                <div className="divide-y divide-[#E4E4EB] border-t-1 border-[#E4E6ED] text-[16px] text-[#646D89] font-normal md:pt-4 md:pb-2 max-h-[468px] overflow-y-auto">
                  {isLoadingTable
                    ...
                    : dataHistoryPayment.map((item, index) => (
                        <div key={index} className={`flex items-center py-4 rounded-md px-4 ${index % 2 !== 0 ? "bg-[#F5F5FA]" : ""}`}>
                ...
                <div className="flex justify-start pl-4 pt-2 pb-4 border-t border-[#E4E4EB] mt-4 md:pt-6 md:items-center md:justify-end">
                  <button
                    ...
                    className="text-[#C70039] font-bold text-sm md:text-[16px] md:pr-2 hover:underline cursor-pointer"
                  >
                    Request PDF
                  </button>
                </div>
